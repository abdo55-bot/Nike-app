  ...
  return (
    <div className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ifExists?'justify-items-start ':'justify-items-center'}   rounded-xl py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105`}>
      <div className={`grid items-center ${ifExists?'justify-items-start ':'justify-items-center'}`} >
        ...
        <button type='button' className='bg-white/90 blur-effect-theme button-theme p-0.5 shadow-slate-200'><ShoppingBagIcon className='icon-style text-slate-900'/></button>
        <button type='button' className='bg-white/90 blur-effect-theme button-theme px-2 py-1 shadow-slate-200 text-sm text-black'>{btn} </button>
      ...
      <img src={img} alt={`img/item ${id}`} className={` transitions-theme
